...
class NN:
    ...
    def backPropagate(self, targets, N, M):
        ...
        for k in range(self.no):
            ...
            output_deltas[k] = dsigmoid(self.ao[k]) * error  #误差对y求导 乘 y对未sigmoid的输入求导，递推公式
        ...
        for j in range(self.nh):
            error = 0.0
            for k in range(self.no):
                error = error + output_deltas[k]*self.wo[j][k]   #计算WX+b对x的导数，也就是self.wo[j][k] ,再乘上一层的梯度
    ...
    def train(self, patterns, iterations=1000, N=0.1, M=0.01):
        ...
        for i in range(iterations):
            ...
            for p in patterns:        #这个for执行了100次
                #print p
                inputs = p[0]
                targets = p[1]
                # print targets
                self.update(inputs)  #计算前向传播
...
def iris():
   ...
   # 随机排列data
    random.shuffle(data)
    # print data
    training = data[0:100]  #100份训练集
    test = data[101:]   #50份测试集
    # print np.shape(l)
    # print np.shape(data)
